...
WeatherStation::WeatherStation()
    : _bh1750(0x23), _inaSolar(INA226_ADDR_SOLAR),
      _inaBattery(INA226_ADDR_BATTERY), _mqtt(_wifiClient),
...
void WeatherStation::onWifiEvent(void *arg, esp_event_base_t base, int32_t id,
                                 void *data) {
...
void WeatherStation::onIpEvent(void *arg, esp_event_base_t base, int32_t id,
                               void *data) {
...
void WeatherStation::onMqttMessage(char *topic, uint8_t *payload,
                                   unsigned int len) {
  if (!s_instance)
    return;
  String t(topic);
...
void WeatherStation::connectWifi() {
  ...
  esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID,
                             &WeatherStation::onWifiEvent, this);
  esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP,
                             &WeatherStation::onIpEvent, this);

  wifi_config_t wifi_cfg = {};
  strncpy(reinterpret_cast<char *>(wifi_cfg.sta.ssid), SECRET_WIFI_SSID,
          sizeof(wifi_cfg.sta.ssid) - 1);
  strncpy(reinterpret_cast<char *>(wifi_cfg.sta.password), SECRET_WIFI_PASSWORD,
...
bool WeatherStation::connectMqtt() {
  ...
  for (int i = 0; i < MAX_ATTEMPTS; ++i) {
    ...
    if (_mqtt.connect(MQTT_CLIENT_ID, nullptr, nullptr, MQTT_TOPIC_AVAIL, 0,
                      true, MQTT_PAYLOAD_OFFLINE)) {
...
SensorData WeatherStation::readSensors() {
  ...
  d.solarPanelCurrent =
      fmaxf(0.0f, _inaSolar.getCurrent()); // clamp negative noise to 0
...
void WeatherStation::runOtaMode() {
  ...
  _server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    request->send(200, "text/html", ota_ui::kPageHtml);
  });
  ...
  _server.on("/reboot", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    ESP_LOGW(TAG, "Reboot requested from OTA UI");
  ...
  ESP_LOGI(TAG, "OTA UI ready at http://<ip>/ (reboot + link to /update)");
  ESP_LOGI(TAG, "ElegantOTA ready at http://<ip>/update");

  const TickType_t EMIT_PERIOD_TICKS =
      pdMS_TO_TICKS(SLEEP_DURATION_US / 1000ULL);
  TickType_t lastEmitTick =
      xTaskGetTickCount() - EMIT_PERIOD_TICKS; // trigger immediate first emit
...
bool WeatherStation::waitBits(EventBits_t bits, uint32_t timeoutMs) {
  EventBits_t result = xEventGroupWaitBits(_events, bits, pdFALSE, pdTRUE,
