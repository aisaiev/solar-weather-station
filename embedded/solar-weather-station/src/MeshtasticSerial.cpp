...
void MeshtasticSerial::sendEnvironmentTelemetry(const SensorData& d) {
    ...
    // Meshtastic clients).
    n = 0;
    if (!isnan(d.temperature)) n += writeFloat(env + n, 1, d.temperature);
    if (!isnan(d.humidity))    n += writeFloat(env + n, 2, d.humidity);
    if (!isnan(d.pressure))    n += writeFloat(env + n, 3, d.pressure);
    if (!isnan(d.illuminance)) n += writeFloat(env + n, 9, d.illuminance);
    ...
    n = 0;
    n += writeFixed32(pkt + n, 2, 0xFFFFFFFFu);   // to = BROADCAST_ADDR (fixed32, field 2)
    ...
    ESP_LOGI(TAG, "Sent environment telemetry: temp=%.1f°C hum=%.1f%% pres=%.1fhPa lux=%.0f (%u payload bytes)",
...
size_t MeshtasticSerial::writeVarintField(uint8_t* buf, uint8_t field, uint64_t v) {
...
size_t MeshtasticSerial::writeLenDelim(uint8_t* buf, uint8_t field,
    ...
    buf[0] = static_cast<uint8_t>((field << 3u) | 2u); // wire type 2 = length-delimited
